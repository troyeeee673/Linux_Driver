...
static ssize_t imx6uirq_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
    ...
    /*等待事件(等待按键值有效)*/
#if 0
    wait_event_interruptible(dev->r_wq, atomic_long_read(&dev->release_key));
#endif
    DECLARE_WAITQUEUE(wait, current); // 定义一个等待队列项
    /*未按下*/
    if (atomic_long_read(&dev->release_key) == 0)
    {
        // 向等待队列中添加队列项
        ...
        remove_wait_queue(&dev->r_wq, &wait); /* 将对应的队列项从等待队列头删除 */
    }
...
static void timer_func(unsigned long arg)
{
    ...
    /*唤醒进程*/
    if (atomic_long_read(&dev->release_key))
    {
        wake_up(&dev->r_wq);
    }
}
...
static int __init imx6uirq_init(void)
{
    ...
    /*等待队列头*/
    init_waitqueue_head(&imx6uirqdev.r_wq);
