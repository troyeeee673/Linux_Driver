...
static int led_open(struct inode *inode, struct file *filp)
{
    ...
    /*判断原子变量是否被锁*/
    if(!atomic_long_dec_and_test(&gpioled.lock))//如果lock减1后为0表示真，则表示已经正常获取原子变量，否则-1后不为0， 也就是小于0，则返回错误
    {
        atomic_long_inc(&gpioled.lock);
         return -EBUSY;
    }
       
#if 0
    if(atomic_long_read(&gpioled.lock) <= 0)
    {
        return -EBUSY;
    }
    else atomic_long_dec(&gpioled.lock);
#endif
    return 0;
}
...
static int __init led_init(void)
{
    int ret = 0;
    /*初始化原子变量为1*/
    atomic_long_set(&gpioled.lock, 1);
    /*注册字符驱动*/
    gpioled.major = 0;
    if (gpioled.major)
    {
